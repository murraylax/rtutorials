...
`  geom_bar(fill="lightblue", col="black") +`
sets up the *geometry* layer of the plot. The geometry layer specifies what kind of shape will be plotted. In this case, we plot bars that are shaded light blue and have black outlines.

The last three segments of code,
`  labs(title="Histogram for Importance of Grades") +`
`  xlab("Importance of Grades") + `
`  ylab("Frequency")`
set up the x-axis label, y-axis label, and title of the plot.
