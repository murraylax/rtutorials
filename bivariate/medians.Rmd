...
sets up a plot, establishes the data frame to be used (`df`), and maps the `Grades` variable to the x-axis.  The `as.factor(Grades)` tells R to treat the `Grades` variable as a `factor`, i.e. a categorical variable.
