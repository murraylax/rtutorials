...
[PDF file location: http://www.murraylax.org/rtutorials/regression_intro.pdf](http://www.murraylax.org/rtutorials/regression_intro.pdf)

[HTML file location: http://www.murraylax.org/rtutorials/regression_intro.html](http://www.murraylax.org/rtutorials/regression_intro.html)
...
In the first line above, we call the `ggplot()` function and set the data and aesthetic layers of the graph. We define the data layer by seeting the parameter `data` equal to the `wages` data frame. We define the aesthetics layer with the mapping parameter.  We create a mapping with a call to the `aes()` function and map `YearsEdu` to the x-axis and `MonthlyEarnings` to the y-axis.  

We add the geometery layer in the second line with a call to `geom_point()`. This creates a plot with points on it.
