...
The code below downloads and loads into membeor the data set.
...
```{r}
ahestats <- t.test(ahe ~ female, data=df, conf.level=0.95)
```

The first parameter, `ahe ~ female`, is a *formula* that says we are interested in the outcome variable `ahe` and how it is different for different values for the explanatory variable, `female`.  The second parameter, `data=df`, tells the function in what data frame the variables `ahe` and `female` can be found.  The last parameter, `conf.level=0.95`, will generate output that will be useful later for computing a 95% confidence interval. 
