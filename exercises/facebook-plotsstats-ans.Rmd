...
```{r}
ggplot(df, aes(x=Month, y=EngagedUsers)) + 
...
```{r}
df$August <- df$Month == "August"
ggplot(df, aes(x=Month, y=EngagedUsers, fill=August)) + 
  stat_summary(fun.data=mean_sdl, geom="bar") +
  stat_summary(fun.data=mean_cl_normal, geom="errorbar", width=0.3) +
  theme(legend.position = "none") +
  labs(title="Engaged Users by Month", x="", y="") +
  scale_fill_manual(values = c("#74ADD3", "#515873"))
```
...
```{r}
t.test(EngagedUsers ~ August, data=df)
```

4. Report the interpoloted median and the median number of engaged users by month. Is there evidence that the median number of engaged users from posts made in August is different than other months?

```{r}
df %>%
  group_by(Month) %>%
  summarize(MedianEngagedUsers = median(EngagedUsers))

wilcox.test(EngagedUsers ~ August, data=df)
```
...
```{r}
ggplot(df, aes(x=Comments, y=Impressions)) + geom_point() +
  coord_cartesian(xlim=c(0,30), ylim=c(0,60000)) +
  geom_smooth(method="lm") +
  labs(title="Relationship between Comments and Impressions")
```